...
const MovieCard = memo(({ movie, comingSoon }: MovieCardProps) => {
  ...
  const cardContent = (
    <div className="relative aspect-[2/3] rounded-lg sm:rounded-xl overflow-hidden mb-1.5 sm:mb-2 gpu-layer">
      ...
      {isFuture ? (
        <div className="absolute bottom-1 left-1 sm:bottom-1.5 sm:left-1.5 flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-primary/80 text-primary-foreground text-[7px] sm:text-[8px] font-bold uppercase tracking-wider">
  ...
  if (isFuture) {
    return (
      <>
        <div className="group flex-shrink-0 w-full block cursor-pointer" onClick={() => setShowModal(true)}>
          {cardContent}
        ...
        {showModal && <ComingSoonModal movie={movie} onClose={() => setShowModal(false)} />}
      </>
    );
  }
